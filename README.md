...
Stored at `~/.discli/config.json` (auto-secured with `0600` permissions). Token resolution: `--token` flag → `DISCORD_BOT_TOKEN` env var → config file → `DISCORD_TOKEN` env var.
