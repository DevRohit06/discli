...
All voice actions require the `voice` extra. Most accept either `channel_id` or `guild_id` for routing — when only `channel_id` is given, the guild owning that voice channel is resolved automatically. See the [Voice guide](/guides/voice) for the user-facing tutorial.
...
| Field | Required | Type | Description |
|-------|----------|------|-------------|
| `channel_id` | yes | string | Voice channel ID |
| `req_id` | no | string | Correlation ID |
...
Move the bot to a different voice channel in the same guild.
...
Synthesise text via TTS and play it.
...
| Field | Required | Type | Description |
|-------|----------|------|-------------|
| `channel_id` or `guild_id` | yes | string | Voice channel or guild ID |
| `text` | yes | string | Text to synthesise |
| `voice` | no | string | TTS voice ID (provider-specific) |
| `speed` | no | number | Speech rate multiplier (default `1.0`) |
| `tts` | no | string | Override TTS provider (`elevenlabs`, `openai`, `deepgram`) |

### voice_play

Play an audio file or URL via ffmpeg.
...
|-------|----------|------|-------------|
| `channel_id` or `guild_id` | yes | string | Voice channel or guild ID |
| `source` | yes | string | File path or HTTP(S) URL |
...
Begin streaming incoming audio to STT. Emits `voice_speech_detected` events.
...
|-------|----------|------|-------------|
| `channel_id` or `guild_id` | yes | string | Voice channel or guild ID |
| `stt` | no | string | Override STT provider (`deepgram`, `openai`) |

### voice_listen_stop

Stop the listener for this guild.
...
Update voice engine config (provider defaults, VAD thresholds, playback volume) without reconnecting.
