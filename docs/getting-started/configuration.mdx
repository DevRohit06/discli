...
| 3 | Config file (`~/.discli/config.json`) | `discli config set token YOUR_TOKEN` |
| 4 (lowest) | `DISCORD_TOKEN` environment variable | `export DISCORD_TOKEN=your_token` |

This means a `--token` flag always wins, followed by `DISCORD_BOT_TOKEN`, followed by the saved config file, and finally `DISCORD_TOKEN` as a generic fallback. Use whichever method fits your workflow.
...
<Callout type="note">
  On macOS and Linux, discli automatically creates `~/.discli/config.json` with restricted `0600` permissions and sets `~/.discli/` to `0700` so other local users cannot read your token.
</Callout>
