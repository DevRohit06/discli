...
For select menus, `values` contains the selected option values.
</Accordion>
...
The `action` field is one of `join`, `leave`, or `move`.
</Accordion>
...
**Rich embeds:** Include an `embed` object on `send` or `reply` actions:
...
**Message components:** Include a `components` array to attach buttons and select menus:
