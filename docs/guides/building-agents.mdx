...
**What could go wrong:** Every `reply()` call spawns a new discli process, which starts a Python interpreter, authenticates a fresh HTTP client, sends the message, and exits. There is no Gateway handshake (see [Connection Model](/architecture/connection-model)), but process startup and a cold HTTP session still dominate the cost, and each reply pays it again. Fine for low-traffic bots, but it will not scale.
</Callout>
