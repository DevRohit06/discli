...
- `discli listen` and `discli voice` commands *do* open gateway connections, so they contend for the same single-connection slot as `discli serve`. Do not run them against a token that already has a `serve` attached — see [Connection Model](/architecture/connection-model)
