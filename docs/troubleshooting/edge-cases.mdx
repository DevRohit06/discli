...
- For **CLI usage**, file attachments work via the `--file` flag:
  ```bash
  discli message send "#general" "Here's the report" --file ./report.csv
  ```
- For **serve mode**, this is a known limitation. Text-only messages are supported. Rich embeds and file attachments in the JSONL protocol are planned for a future release
