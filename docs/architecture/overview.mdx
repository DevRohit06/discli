...
5. One-shot commands authenticate an HTTP client and execute directly. Live commands connect a Gateway client with feature-specific intents (see [Connection Model](/architecture/connection-model)).
...
| **Serve** | `commands/serve.py` | Persistent bot mode. Reads JSONL from stdin, writes JSONL to stdout. ~2,200 lines covering 65+ actions, event forwarding, slash command registration, and streaming with 1.5s flush intervals. |
...
`voice` commands are the exception to this mode: they need live voice state, so they open a
Gateway session scoped to the `voice` feature and close it when the action finishes. See
[Connection Model](/architecture/connection-model) for the full REST/Gateway split and the
intents each feature requests.
